...
#   commit 69c7c3e8a88c3103d75bb298c3124e66bb79e7ba
#
# AKDNS Wizard modifications:
# - Added a machine-readable JSON mode for akdns-wizard.py via -J.
# - Kept the original human-readable checks available for direct use.
# - Preserved upstream service probes as much as possible; local changes are
#   limited to integration and structured-result output needed by this project.
#
...
while getopts ":I:M:EJX:P:F:S:R:C:D:" optname; do
    case "$optname" in
        "I")
            iface="$OPTARG"
...
akdns_json_result() {
    ...
    if echo "$output" | grep -Eqi 'Failed|Network Connection|Unexpected Result|Unsupport|IPv6 Not Support|error'; then
        status="error"
    fi
    if echo "$output" | grep -Eqi '(^|[^A-Za-z])(No|Banned|Unavailable|Forbidden|Proxy/VPN Detected|CountryNotAllowed|not available|failed to fetch)([^A-Za-z]|$)'; then
        status="no"
    fi
    if echo "$output" | grep -Eqi 'Oversea Only|Available For|Originals Only|自制剧'; then
        status="partial"
    fi
    if echo "$output" | grep -Eqi '(^|[^A-Za-z])(Yes|Allow|Success)([^A-Za-z]|$)'; then
        status="yes"
    fi
    if echo "$output" | grep -Eqi 'Steam Currency:'; then
        if echo "$output" | grep -Eqi 'Failed|Network Connection'; then
            status="error"
        else
            status="yes"
        fi
    fi
    if echo "$output" | grep -Eqi 'YouTube CDN|Netflix Preferred CDN|Google Search Region|Google Region|YouTube Region|Wikipedia Editability'; then
        if echo "$output" | grep -Eqi 'Failed|Network Connection|No'; then
            status="no"
        elif [ -n "$output" ]; then
            status="yes"
        fi
    ...
    region=$(echo "$output" | sed -nE 's/.*\((Region|Country|Currency):[[:space:]]*([^)]*)\).*/\2/ip' | head -n 1 | tr -d '\r')
    if [ -z "$region" ]; then
        region=$(echo "$output" | sed -nE 's/.*(Region|Country):[[:space:]]*([A-Za-z0-9_ -]+).*/\2/ip' | head -n 1 | tr -d '\r')
    fi

    local escaped_detail
    escaped_detail=$(printf '%s' "$detail" | akdns_json_escape)
    local escaped_region
    escaped_region=$(printf '%s' "$region" | akdns_json_escape)
    printf '{"function":"%s","ip_version":"%s","status":"%s","region":%s,"detail":%s}\n' "$func_name" "$ip_version" "$status" "$escaped_region" "$escaped_detail"
}
...
akdns_run_json_func() {
    ...
    if ! declare -F "$func_name" >/dev/null; then
        printf '{"function":"%s","ip_version":"%s","status":"error","region":"","detail":"unknown function"}\n' "$func_name" "$ip_version"
        return 1
    fi
    local output
    output=$($func_name "$ip_version" 2>&1)
    akdns_json_result "$func_name" "$ip_version" "$output"
}

if [ -n "$func" ]; then
    if [ "$akdns_json" = 1 ]; then
        if [[ "$NetworkType" == "4" ]]; then
            akdns_run_json_func "$func" 4
        elif [[ "$NetworkType" == "6" ]]; then
            akdns_run_json_func "$func" 6
        else
            akdns_run_json_func "$func" 4
            akdns_run_json_func "$func" 6
        fi
